...
var localStorageKeyNames = {temp: 'temporaryUser', user: 'user'}
...
function configureBaseUser(io, socket, data, type){
  ...
  socket.room ="General";
  var key = type === 'temp' ? localStorageKeyNames.temp : localStorageKeyNames.user
  socket.emit('save user in localstorage', {key: key, user: data.user, color: data.color})
