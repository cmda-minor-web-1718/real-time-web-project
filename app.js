...
io.on('connection', function (socket) {
    ...
    socket.on('joinRoom', function (room) {
        ...
        getLeTweetStream(room)
        getLeTweets(room)
    })
        
    const getLeTweets = function(room) {
        client.get('search/tweets', { q: '#' + room, result_type: 'recent', count: "30" }, function (error, tweets, response) {

            const allTweets = tweets.statuses
            io.emit('loadStaticTweets', allTweets)
        })
    }

    const getLeTweetStream = function(room) {
        ...
        newTwitterStream.on('data', function (obj) {
            ...
            io.to(room).emit('newTweet', result)
        })
